...
const networkDelayTime = (times, n, k) => {
  ...
  while (pq.length > 0) {
    ...
    for (const neighbor of neighbors) {
      const [neighborNode, neighborTime] = neighbor
...
const main = () => {
  ...
  const n = [4, 5, 4, 5, 2, 75]
  const k = [3, 1, 1, 1, 2, 19]

  for (let i = 1; i < 76; i++) {
    console.log(
      `Minimum amount of time required for node ${i} to send the signal = `,
      networkDelayTime(times[5], 75, i) + 'ms\n'
    )
  }

  // for (let i = 0; i < times.length; ++i) {
  //   console.log(`${i + 1}.\t times = [${times[i].map(t => `[${t.join(', ')}]`).join(', ')}]`);
  //   console.log(`\t number of nodes 'n' = ${n[i]}`);
  //   console.log(`\t starting node 'k' = ${k[i]}\n`);
  //   console.log(`\t Minimum amount of time required = ${networkDelayTime(times[i], n[i], k[i])}`);
